...
const DERIVED = [
  ...
  {id:"bom",         name:"Lista de materiais (BOM)",           unit:"relatório", hh:2.0},
  {id:"clash",       name:"Relatório de clash detection",       unit:"rodada",    hh:3.0},
  {id:"federado",    name:"Modelo federado / coordenação",      unit:"disciplina",hh:4.0},
];
...
const DEFAULT_SETTINGS = {
  ...
  margemEnabled:true, margemMaquete:10, margemProjeto3d:15, margemBim:20,
  ...
  escalonamento:{
    enabled:true,
    faixas:[
      {id:"f1",label:"Até 50 A1",      upTo:50,   fator:1.00},
      {id:"f2",label:"51 a 100 A1",    upTo:100,  fator:0.90},
      {id:"f3",label:"Acima de 100 A1",upTo:9999, fator:0.80},
    ]
  }
};
...
function SettingsModal({settings,onSave,onClose}){
  ...
  const setFaixa=(id,k,v)=>setS(p=>({...p,escalonamento:{...p.escalonamento,faixas:p.escalonamento.faixas.map(f=>f.id===id?{...f,[k]:v}:f)}}));
      ...
      <div style={{background:"#fff",borderRadius:20,width:"100%",maxWidth:680,maxHeight:"92vh",overflowY:"auto",boxShadow:"0 24px 64px rgba(0,0,0,.18)"}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"16px 24px",borderTop:`1px solid ${C.mid}`}}>
          ...
          <div style={{display:"flex",gap:10}}>
            ...
            <button onClick={()=>{onSave(s);onClose();}} style={{padding:"9px 22px",borderRadius:10,border:"none",background:C.teal,color:"#fff",fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:"Outfit, sans-serif"}}>
  ...
  return(
    <div style={{maxWidth:820,margin:"0 auto",padding:"32px 32px 48px"}}>
      ...
      <div style={{background:C.light,borderRadius:12,padding:"14px 18px",marginBottom:8,border:`1px solid ${C.mid}`}}>
        <p style={{fontSize:13,color:C.text,lineHeight:1.6}}><strong>Exemplo:</strong> Para 120 A1 com faixas padrão (≤50: ×1,00 | 51–100: ×0,90 | +100: ×0,80):
        os primeiros 50 A1 têm fator 1,00, os próximos 50 têm 0,90, os últimos 20 têm 0,80.
        O fator blended resultante é <strong>(50×1,00 + 50×0,90 + 20×0,80) / 120 = 0,925</strong>, reduzindo as horas totais em 7,5%.</p>
      </div>
      ...
      <p style={{fontSize:13,color:C.muted,marginBottom:28,lineHeight:1.6}}>
        Utilize o botão <strong>"Salvar orçamento"</strong> na tela de Resultado para armazenar o orçamento localmente no navegador.
        ...
        <br/><strong>Atenção:</strong> os dados são salvos no armazenamento local do navegador. Limpar os dados do navegador ou trocar de dispositivo apagará os orçamentos.
      </p>
    ...
    <div style={{maxWidth:720,margin:"0 auto",padding:"32px"}}>
      ...
      <div style={{marginBottom:18,paddingBottom:16,borderBottom:`1.5px solid ${C.mid}`}}>
        ...
        <p style={{fontSize:12,color:C.muted,marginTop:4}}>
          {quotes.length} orçamento{quotes.length!==1?"s":""} armazenado{quotes.length!==1?"s":""} neste navegador.
          <span style={{color:C.border,marginLeft:6}}>Dados acessíveis apenas neste dispositivo/navegador.</span>
        </p>
      </div>
...
function Step1({data,onChange}){
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      ...
      {fld("Elaborado por","responsavel","Nome do responsável comercial")}
      <div>
...
function Step5({data,onChange,totalA1}){
  const toggle=id=>{
    const exists=data.find(x=>x.id===id);
    if(exists){onChange(data.filter(x=>x.id!==id));}
    else{onChange([...data,{id,qty:id==="documentacao"?Math.max(1,totalA1):1}]);}
  };
...
export default function App(){
  ...
  const sideBtn=(label,icon,v)=>(
    <button onClick={()=>setView(v)} style={{display:"flex",alignItems:"center",gap:10,padding:"9px 10px",borderRadius:10,background:view===v?`${C.teal}33`:"none",border:view===v?`1px solid ${C.teal}`:"1px solid transparent",cursor:"pointer",width:"100%",transition:"all .15s"}}>
      {icon}
        ...
        <div style={{flex:1,overflowY:"auto"}}>
          ...
          {view==="wizard"&&(
            <div style={{maxWidth:700,margin:"0 auto",padding:"32px 32px 24px"}}>
              ...
              {step===5&&<Step5 data={derivados} onChange={setDerivados} totalA1={totalA1}/>}
