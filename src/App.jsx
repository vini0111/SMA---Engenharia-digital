...
const DISCIPLINES = [
  ...
  {id:"iluminacao",        name:"Iluminação e tomadas",              hhBase:8},
  {id:"aterramento",       name:"Aterramento e SPDA",                hhBase:6},
  {id:"arranjo_sala",      name:"Arranjo de sala elétrica",          hhBase:6},
  {id:"rede_aerea",        name:"Rede aérea",                        hhBase:8},
  {id:"estrutural",        name:"Estrutural (fund. / forma / arm.)", hhBase:6},
  {id:"metalica",          name:"Estrutura metálica",                hhBase:8},
  {id:"arquitetura",       name:"Arquitetura",                       hhBase:6},
...
const DERIVED = [
  {id:"documentacao",name:"Documentação / Revit nativo",       unit:"formato",     hh:4.0},
  {id:"dwg",         name:"Exportação / DWG",                   unit:"formato",     hh:2.0},
  {id:"bom",         name:"Lista de materiais (BOM)",           unit:"disciplinas", hh:2.0},
  {id:"clash",       name:"Relatório de clash detection",       unit:"disciplinas", hh:4.0},
...
const DEFAULT_SETTINGS = {
  ...
  licencaAnual:15673.25, horasAnuais:2088,
  ...
  engEnabled:true,
  engHH:{
    forca:6.0, iluminacao:4.0, aterramento:4.0, arranjo_sala:8.0,
    rede_aerea:4.0, estrutural:4.0, metalica:4.0, arquitetura:4.0, drenagem:4.0
  },
  // When engEnabled=true, modelador HH increases (Tabela 2)
  disciplinesWithEng:{
    forca:12, iluminacao:12, aterramento:10, arranjo_sala:10,
    rede_aerea:12, estrutural:10, metalica:12, arquitetura:10, drenagem:10
  },
  derivedHH:{documentacao:4.0,dwg:2.0,bom:2.0,clash:4.0,federado:4.0},
  ...
  escalonamento:{
    ...
    faixas:[
      ...
      {id:"f2",label:"21 a 50 A1",         upTo:50,   fator:0.85},
      {id:"f3",label:"51 a 100 A1",        upTo:100,  fator:0.65},
      {id:"f4",label:"Acima de 101 A1",    upTo:9999, fator:0.50},
    ]
  }
};
const initDisciplines = s => DISCIPLINES.map(d=>{
  // When engEnabled, use the higher modelador HH from disciplinesWithEng (Tabela 2)
  // When disabled, use the base HH from disciplines (Tabela 1)
  const hhVal = s.engEnabled
    ? (s.disciplinesWithEng?.[d.id] ?? s.disciplines[d.id] ?? d.hhBase)
    : (s.disciplines[d.id] ?? d.hhBase);
  return {...d,active:false,a1:1,hh:hhVal};
});
...
function SettingsModal({settings,onSave,onReset,onClose}){
  const [s,setS]=useState({...settings,
    ...
    disciplinesWithEng:{...settings.disciplinesWithEng},
  ...
  const setDWE=(id,v)=>setS(p=>({...p,disciplinesWithEng:{...p.disciplinesWithEng,[id]:v}}));
  ...
  const resetAll=()=>{
    const def={...DEFAULT_SETTINGS,disciplines:{...DEFAULT_SETTINGS.disciplines},disciplinesWithEng:{...DEFAULT_SETTINGS.disciplinesWithEng},engHH:{...DEFAULT_SETTINGS.engHH},derivedHH:{...DEFAULT_SETTINGS.derivedHH},complexityX:{...DEFAULT_SETTINGS.complexityX},lodX:{...DEFAULT_SETTINGS.lodX},loiX:{...DEFAULT_SETTINGS.loiX},escalonamento:{...DEFAULT_SETTINGS.escalonamento,faixas:DEFAULT_SETTINGS.escalonamento.faixas.map(f=>({...f}))}};
    setS(def);
      ...
      <div style={{background:"#fff",borderRadius:20,width:"100%",maxWidth:680,maxHeight:"92vh",overflowY:"auto",boxShadow:"0 24px 64px rgba(0,0,0,.18)"}}>
        ...
        <div style={{padding:24,display:"flex",flexDirection:"column",gap:22}}>
          ...
          <section>
            ...
            {/* Column headers */}
            <div style={{display:"grid",gridTemplateColumns:s.engEnabled?"1fr 90px 90px 90px":"1fr 90px",gap:8,marginBottom:4}}>
              <span/>
              <p style={{fontSize:10,fontWeight:700,color:C.teal,textTransform:"uppercase",letterSpacing:".05em",textAlign:"center"}}>{s.engEnabled?"Modelador (Tab.1)":"Modelador"}</p>
              {s.engEnabled&&<p style={{fontSize:10,fontWeight:700,color:"#1D6E5F",textTransform:"uppercase",letterSpacing:".05em",textAlign:"center"}}>Modelador (Tab.2)</p>}
              {s.engEnabled&&<p style={{fontSize:10,fontWeight:700,color:"#7C3AED",textTransform:"uppercase",letterSpacing:".05em",textAlign:"center"}}>Engenheiro</p>}
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {DISCIPLINES.map(d=>(
                <div key={d.id} style={{display:"grid",gridTemplateColumns:s.engEnabled?"1fr 90px 90px 90px":"1fr 90px",gap:8,alignItems:"center"}}>
                  ...
                  {s.engEnabled&&<FormattedInput value={s.disciplinesWithEng?.[d.id]??s.disciplines[d.id]} onChange={v=>setDWE(d.id,v)} decimals={1} style={{...iStyle,textAlign:"center",border:`1.5px solid #6BAA8C`}}/>}
                  {s.engEnabled
  ...
  return(
    <div style={{maxWidth:820,margin:"0 auto",padding:"32px 32px 48px"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:24}}>
        {card("Área de aplicação","Departamento Comercial e DEP (Diretoria de Engenharia de Projetos), para uso interno na composição de orçamentos.")}
      </div>
      ...
      {/* Row 3: Fator multiplicador | HH padrão */}
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}}>
      ...
      <div style={{background:C.light,borderRadius:12,padding:"14px 18px",marginBottom:8,border:`1px solid ${C.mid}`}}>
        <p style={{fontSize:13,color:C.text,lineHeight:1.6}}><strong>Exemplo:</strong> Para 120 A1 com as faixas padrão (≤20: ×1,00 | 21–50: ×0,85 | 51–100: ×0,65 | +101: ×0,50):
        os primeiros 20 A1 têm fator 1,00; os próximos 30 (21–50) têm 0,85; os próximos 50 (51–100) têm 0,65; os últimos 20 (101–120) têm 0,50.
        O fator de escalonamento resultante é <strong>(20×1,00 + 30×0,85 + 50×0,65 + 20×0,50) / 120 ≈ 0,733</strong>, reduzindo as horas totais de modelagem em ~27%.</p>
      </div>
...
function ScenarioComparatorModal({scenarioA,calcA,scenarioB,calcB,settings,onClose}){
  ...
  const handlePDF=()=>{
    ...
    *{box-sizing:border-box;margin:0;padding:0;}
    ...
    <div class="footer">
      <span>SM&amp;A Sistemas Elétricos e Automação &nbsp;|&nbsp; DEP — Diretoria de Engenharia de Projetos</span>
...
<table><thead><tr>
  ...
  ${(engHours>0)?`<tr><td>${settings.engEnabled?"Engenharia (HH Eng. + HH Modelador)":"Engenharia (HH Eng.)"}</td><td style="text-align:right">${N1(engHours)} h</td><td style="text-align:right">${BRL(engCost)}</td></tr>`:""}
...
// Tooltips por categoria e nível
const TOOLTIPS = {
  complexidade:{
    baixa:     "Multiplicador 1,0 — esforço padrão. Área nova sem restrições, roteamento direto e mínimo retrabalho.",
    media:     "Multiplicador 1,1 — +10 % de esforço. Planta existente com alguma infraestrutura que exige compatibilização.",
    alta:      "Multiplicador 1,3 — +30 % de esforço. Brownfield denso: muitas interferências, soluções de desvio complexas.",
    muito_alta:"Multiplicador 1,4 — +40 % de esforço. Espaço extremamente congestionado, tolerâncias mínimas e engenharia customizada.",
  },
  lod:{
    lod200:"Multiplicador 1,0 — esforço padrão. Elementos esquemáticos: dimensões aproximadas e posicionamento genérico.",
    lod300:"Multiplicador 1,1 — +10 % de esforço. Geometria precisa para compatibilização e retirada de interferências.",
    lod350:"Multiplicador 1,3 — +30 % de esforço. Detalhamento executivo: conexões, suportes e interfaces de sistemas.",
    lod400:"Multiplicador 1,4 — +40 % de esforço. Nível de fabricação com informações completas para montagem em campo.",
  },
  loi:{
    basico:       "Multiplicador 1,0 — esforço padrão. Apenas geometria; nenhum dado não-gráfico preenchido.",
    intermediario:"Multiplicador 1,2 — +20 % de esforço. TAG, material e dimensões; permite extração de BOM básica.",
    avancado:     "Multiplicador 1,3 — +30 % de esforço. Dados completos de ciclo de vida: fabricante, especificação técnica e integração com gestão de ativos.",
  },
};

function Step4({data,onChange,deliveryType,settings}){
  const [showLodImg,setShowLodImg]=useState(false);
  const [tooltip,setTooltip]=useState(null); // {key, text, top, left}
  ...
  const mult=R((settings.complexityX[data.complexidade]||0)*(settings.lodX[data.lod]||0)*(settings.loiX[data.loi]||0),2);

  const selCard=(label,m,desc,active,onClick,color,tooltipText,tooltipKey)=>(
    <button onClick={onClick} style={{position:"relative",textAlign:"left",padding:"12px 14px",borderRadius:12,border:`2px solid ${active?color:C.border}`,background:active?`${color}18`:"#fff",cursor:"pointer",transition:"all .15s"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:desc?4:0}}>
        ...
        <span style={{position:"relative",display:"inline-flex",alignItems:"center",gap:4}}>
          ...
          {tooltipText&&(
            <span
              onMouseEnter={e=>{e.stopPropagation();setTooltip({key:tooltipKey,text:tooltipText});}}
              onMouseLeave={()=>setTooltip(null)}
              onClick={e=>e.stopPropagation()}
              style={{display:"inline-flex",alignItems:"center",justifyContent:"center",width:16,height:16,borderRadius:"50%",background:active?color:C.mid,color:active?"#fff":C.muted,fontSize:10,fontWeight:700,cursor:"help",flexShrink:0,userSelect:"none"}}
            >?</span>
          )}
          {/* Tooltip balloon */}
          {tooltipText&&tooltip?.key===tooltipKey&&(
            <div style={{position:"absolute",bottom:"calc(100% + 8px)",right:0,zIndex:80,background:C.dark,color:"#fff",fontSize:11,lineHeight:1.55,padding:"10px 13px",borderRadius:10,width:230,pointerEvents:"none",boxShadow:"0 4px 16px rgba(0,0,0,.22)"}}>
              {tooltipText}
              <div style={{position:"absolute",bottom:-5,right:14,width:10,height:10,background:C.dark,transform:"rotate(45deg)"}}/>
            </div>
          )}
        </span>
      </div>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:20}}>
      ...
      <div><SectionLabel>Nível de complexidade</SectionLabel>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          {COMPLEXITY.map(c=>selCard(c.name,settings.complexityX[c.id]??c.x,c.desc,data.complexidade===c.id,()=>onChange({...data,complexidade:c.id}),C.teal,TOOLTIPS.complexidade[c.id],`cx-${c.id}`))}
        </div>
      </div>
      <div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          {availLOD.map(l=>selCard(l.name,settings.lodX[l.id]??l.x,l.sub,data.lod===l.id,()=>onChange({...data,lod:l.id}),"#3B5BA5",TOOLTIPS.lod[l.id],`lod-${l.id}`))}
        </div>
      </div>
      <div><SectionLabel>LOI — nível de informação</SectionLabel>
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          {availLOI.map(l=>selCard(l.name,settings.loiX[l.id]??l.x,l.desc,data.loi===l.id,()=>onChange({...data,loi:l.id}),"#7C3AED",TOOLTIPS.loi[l.id],`loi-${l.id}`))}
        </div>
      </div>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:20}}>
      ...
      <div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:6}}>
          ...
          {engHours>0&&costRow(settings.engEnabled?"Engenharia (HH Eng. + HH Modelador)":"Engenharia (HH Eng.)",engHours,engCost,"#F0EBF8","#C4B5FD")}
...
function LoginView({onLogin}){
  ...
  return(
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:C.dark,padding:20}}>
      ...
      <p style={{fontSize:11,color:"#2A5A6E",marginTop:24}}>{VERSION} · DEP — Diretoria de Engenharia de Projetos</p>
    </div>
  );
}
...
function loadUserSettings(uid){
  try{
    ...
    return {
      ...
      disciplinesWithEng:{...DEFAULT_SETTINGS.disciplinesWithEng,...(parsed.disciplinesWithEng||{})},
...
export default function App(){
  ...
  const handleLogin=user=>{
    ...
    if(saved){
      setSettings(saved);
      setDisciplines(ds=>ds.map(d=>{
        const hhVal = saved.engEnabled
          ? (saved.disciplinesWithEng?.[d.id] ?? saved.disciplines[d.id] ?? d.hhBase)
          : (saved.disciplines[d.id] ?? d.hhBase);
        return {...d,hh:hhVal};
      }));
    }
  };
  ...
  const handleResetSettings=defSettings=>{
    ...
    setDisciplines(ds=>ds.map(d=>{
      const hhVal = defSettings.engEnabled
        ? (defSettings.disciplinesWithEng?.[d.id] ?? defSettings.disciplines[d.id] ?? d.hhBase)
        : (defSettings.disciplines[d.id] ?? d.hhBase);
      return {...d,hh:hhVal};
    }));
  };
  ...
  const multiplier=useMemo(()=>R(
    ...
    (settings.loiX[params.loi]||0),2),[params,settings]);
  ...
  const calc=useMemo(()=>{
    ...
    // When engEnabled, disciplines use the higher modelador HH (disciplinesWithEng / Tabela 2)
    // When disabled, disciplines use base HH (disciplines / Tabela 1)
    const activeDisciplines=disciplines.filter(d=>d.active);
    const scaledModelingHours=R(Math.round(
      activeDisciplines.reduce((s,d)=>s+d.a1*d.hh*multiplier,0)*scalingFactor
    ),0);
    // Engineering hours: per discipline, scaled by A1, not affected by multiplier or scaling
    const engHours=settings.engEnabled
      ? R(activeDisciplines.reduce((s,d)=>s+(settings.engHH[d.id]??4.0)*d.a1,0),2)
      : 0;
      ...
      <aside style={{width:252,flexShrink:0,display:"flex",flexDirection:"column",background:C.dark,minHeight:"100vh",position:"sticky",top:0,height:"100vh"}}>
        ...
        <div style={{padding:"10px 12px",borderTop:"1px solid rgba(255,255,255,.06)",display:"flex",flexDirection:"column",gap:3}}>
          ...
          <div style={{padding:"10px 10px 6px",borderTop:"1px solid rgba(255,255,255,.06)",marginTop:2}}>
            ...
            <p style={{fontSize:9,color:"#2A5A6E",lineHeight:1.5}}>{VERSION} · DEP — Diretoria de Engenharia de Projetos</p>
          </div>
        </div>
      </aside>
